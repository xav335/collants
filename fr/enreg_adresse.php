...
else
{
	...
	// update nom_client si c'est l'adresse principale
	
	$ChaineSQL = "SELECT num_addresse_defaut FROM client WHERE num_client='".$_SESSION["num_client"]."'";	
	$result = mysql_query($ChaineSQL);
	
	if (mysql_num_rows($result)!=0)
	{
		$row = mysql_fetch_array($result);
		if ($row["num_addresse_defaut"] == $_POST["num_adresse"])
		{
			// c'est bien sarah connor..
			$ChaineSQL = "UPDATE client SET email='".$_POST["email"]."', nom='".$_POST["nom"]."', prenom='".$_POST["prenom"]."', num_civilite='".$_POST["civilite"]."' WHERE num_client='".$_SESSION["num_client"]."'";
			$ma_modif = mysql_query($ChaineSQL);
		}
	}
